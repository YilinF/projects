...
void compute_energy_matrix(const Image* img, Matrix* energy) {
    ...
    for (int i = 1; i < Image_height(img) - 1; i++) {
        for (int j = 1; j < Image_width(img) - 1; j++) {
            // obtain the pixel around the current pixel
            ...
            int energy_x = (squared_difference(north, south)
            + squared_difference(west, east));
            *Matrix_at(energy, i, j) = energy_x;
        }
    }
...
void compute_vertical_cost_matrix(const Matrix* energy, Matrix *cost) {
    ...
    for (int i = 1; i < Matrix_height(energy); i++) {
        for (int j = 0; j < Matrix_width(energy); j++) {
            // special case for leftmost column
            if (j == 0) {
                cost_x = *Matrix_at(energy, i, j) +
                Matrix_min_value_in_row(cost, i - 1, j, j + 2);
            }
            // special case for rightmost column
            else if (j == Matrix_width(energy) - 1) {
                cost_x = *Matrix_at(energy, i, j) +
...
void remove_vertical_seam(Image *img, const int seam[]) {
    ...
    for (int i = 0; i < Image_height(img); i++) {
        ...
        // shrink width by 1, sets column - 1 in aux to current column in img
